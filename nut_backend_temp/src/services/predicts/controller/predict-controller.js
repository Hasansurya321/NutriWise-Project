...
export const predictImage = async (req, res, next) => {
  ...
  try {
    ...
    const { error: storageError } = await supabase.storage.from('food-images').upload(filePath, file.buffer, { contentType: file.mimetype });
    ...
    const createdLog = await PredictRepositories.createLog({
      userId,
...
export const getPredictLogs = async (req, res, next) => {
  ...
  try {
    const [predictLogs, totalLogs] = await Promise.all([PredictRepositories.getPredictLogs(userId, skip, limit, filter), PredictRepositories.countPredictLogs(userId, filter)]);
